...
A fundamental problem working with scientists (in particular physics and astronomy were mentioned), a huge amount of the work is the software, but only the papers count. The process then becomes ``I write the software and then I write a paper as well.'' We need a culture change to make the community aware that scientific software represents intellectual content and scientific innovation directly and explicitly.

Agreeing on the desirable future, we discussed the status quo and ways of changing it. In Computer Science and Bio-Informatics journals exist which are home to intellectual contributions of software packages. Several computer science conference allow for ``artifact evaluation'' where software contributions are scrutinized and appreciated as part of the academic review process. The appreciation of software as academic output does not seem to be universal across domains. 

For computer science the appreciation for software seems natural, hence some attention to making its impact visible. Still this is only marginally so. An important positive factor is the recent focus in research towards ``innovation'' and ``valorization'' which puts tangible and transferable results of Computer Science in the form of working software in the spotlight. Bio-Informatics though, is ahead of the pack, which may be due to its explicit branding as a the informatics branch of Biology. \jv{I couldn't help but add some thoughts to this previous paragraph. Please remove if its too much}

One way of making change is to include software output somehow into the evaluation systems of academics and academic institutes. Currently, publishing software may give you a DOI (e.g. via \url{https://zenodo.org/}) but it does not get you reputation or contribute to a positive evaluation per s\'e. 

Problems with making software output part of the academic process exist. For one software lasts longer than the citable snapshots we have now, which leads to evolving author lists and evolution between versions which may lead to software growing beyond the original scope. Short papers about software are simply that: too short to make the academic challenges and contributions observable. Long papers about software hide the software contributions under the traditional paper-style contributions. And finally, reading and appreciating source code is just hard especially if it is not written originally for an audience.

The solutions we discussed are the following: \jv{based on the notes, but I did use some imagination here:}
\begin{itemize}
	\item Papers could capture more about the software's intellectual impact; i.e. in a separate section akin to the standard ``research method'' and ``threats to validity'' sections.
    \item Letters may be the best way to communicate software impact to tenure committees
    \item People have to claim their contributions more explicitly; web pages about software contributions, research software engineers should publish the papers  and be motivated to do so, and willingness to be proud and certainly unapologetic about software contributions in front of tenure committees. 
    \item Work on rubrics and templates for letters of recommendation to foreground the intellectual contribution from code.
    \item Start a Research Coordination Network (RCN) workshop for how to showcase intellectual content for academic software. 
    \item Move forward to encourage the formation of a prestigious scientific software award. 
\end{itemize}

%------------------------------------------------------------------------
\abstracttitle{Reviewing FORCE11 Software Citation Principles}
\abstractauthor{Dan Katz, et al.\jv{et al. is not nice. who was there?}}

This group came together to contribute to the FORCE11 software citation principles. From the FORCE11 website\footnote{\url{https://www.force11.org/software-citation-principles}} we quote: ``Based on a review of existing community practices, the goal of [FORCE11] was to produce a consolidated set of citation principles that may encourage broad adoption of a consistent policy for software citation across disciplines and venues.''
...
Katz reported recently that this review has been passed on to the FORCE11 Software Citation Working Group editors, leading to updates to the citation principles, where these were deemed necessary.
\jv{Dan could you check the above statement?}
...
\item \emph{Software engineering tools improving productivity by tailoring to intent and skill:} not all research software requires the same development rigour and discipline as commercial software, yet all would benefit from from skills beyond that of ``hobbyist'' programmers. How can we appropriately select and adapt the level of software engineering discipline appropriate to a given research project?
